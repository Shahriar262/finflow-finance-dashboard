...
import { 
  ...
  Home, Zap, CreditCard, ChevronDown, ChevronUp, 
  MoreHorizontal, Smartphone, Car 
} from 'lucide-react';


const getCategoryDetails = (category) => {
  const cats = {
    food: { icon: <Coffee size={18} />, bg: 'bg-orange-50', text: 'text-orange-600' },
    shopping: { icon: <ShoppingBag size={18} />, bg: 'bg-purple-50', text: 'text-purple-600' },
    rent: { icon: <Home size={18} />, bg: 'bg-blue-50', text: 'text-blue-600' },
    utilities: { icon: <Zap size={18} />, bg: 'bg-amber-50', text: 'text-amber-600' },
    salary: { icon: <ArrowUpRight size={18} />, bg: 'bg-emerald-50', text: 'text-emerald-600' },
    transport: { icon: <Car size={18} />, bg: 'bg-slate-50', text: 'text-slate-600' },
    subscription: { icon: <Smartphone size={18} />, bg: 'bg-rose-50', text: 'text-rose-600' },
  };
  return cats[category.toLowerCase()] || { icon: <CreditCard size={18} />, bg: 'bg-slate-50', text: 'text-slate-600' };
};
...
const RecentTransactions = () => {
  ...
  const displayData = isExpanded ? sortedTransactions.slice(0, 10) : sortedTransactions.slice(0, 4);
    ...
    <div className="bg-white rounded-[24px] border border-slate-200/60 shadow-sm overflow-hidden transition-all duration-300">
      ...
      <div className="p-6 border-b border-slate-50 flex items-center justify-between">
        <div>
          ...
          <p className="text-[14px] text-slate-500 font-medium">Real-time transaction monitoring</p>
        </div>
        <button 
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    {/* Icon */}
                    <div className={`w-10 h-10 rounded-xl ${bg} ${text} flex items-center justify-center transition-transform group-hover:scale-105`}>
                      {icon}
                    </div>
                    
                    <div>
                      <p className="font-semibold text-slate-800 text-[14px] md:text-[15px] mb-0.5">{t.title}</p>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className={`font-bold text-[14px] md:text-[16px] ${t.type === 'income' ? 'text-emerald-600' : 'text-slate-900'}`}>
      ...
      {!isExpanded && (
        <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-50 text-center">
          <p className="text-[11px] text-slate-500 font-medium">Verified by FinFlow Security Protocol</p>
        </div>
      )}
    </div>
  );
